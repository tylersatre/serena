...
class MultiLanguageServer:
    def __init__(self, servers: dict[Language, SyncLanguageServer]):
        ...
        log.info(
            "Initialized MultiLanguageServer with %d language servers: %s", len(servers), ", ".join(lang.value for lang in servers.keys())
        )
    ...
    def get_server(self, language: Language) -> SyncLanguageServer:
        if language not in self._servers:
            log.error(
                "Requested language server for %s not found. Available: %s",
                language.value,
                ", ".join(lang.value for lang in self._servers.keys()),
            )
    ...
    def _server_for_path(self, relative_path: str) -> SyncLanguageServer:
        ...
        for lang, server in self._servers.items():
            if lang.get_source_fn_matcher().is_relevant_filename(filename):
                log.debug("Matched %s to %s language server based on filename pattern", relative_path, lang.value)
                return server

        # Fallback to first available server
        fallback_lang = next(iter(self._servers.keys()))
        log.warning("No specific language server matched for %s, falling back to %s", relative_path, fallback_lang.value)
    ...
    def open_file(self, relative_file_path: str) -> Iterator[LSPFileBuffer]:
        ...
        log.info("Opening file %s with %s language server", relative_file_path, lang.value if lang else "fallback")
    ...
    def request_document_symbols(
        ...
        lang = self._language_for_path(relative_file_path)
        log.debug(
            "Requesting document symbols for %s (language: %s, include_body: %s)",
            relative_file_path,
            lang.value if lang else "unknown",
            include_body,
        )
        result = server.request_document_symbols(relative_file_path, include_body)
        log.debug("Retrieved %d top-level and %d total symbols from %s", len(result[0]), len(result[1]), relative_file_path)
        return result
    ...
    def request_full_symbol_tree(
        ...
        log.debug(
            "Requesting full symbol tree (path: %s, language: %s, include_body: %s)",
        ...
        if within_relative_path is not None:
            ...
            log.debug("Using %s language server for path %s", lang.value if lang else "fallback", within_relative_path)
            result = server.request_full_symbol_tree(within_relative_path, include_body)
    ...
    def request_referencing_symbols(
        ...
        log.debug(
            "Requesting references for symbol at %s:%d:%d (source language: %s, filter language: %s)",
            relative_file_path,
            line,
            column,
            ...
            for ref in references:
                ...
                if ref_path:
                    ...
                    if ref_lang == language:
                        ...
                        log.debug("Excluding reference from %s (language: %s)", ref_path, ref_lang.value if ref_lang else "unknown")
            log.info("Filtered references: %d/%d match %s language", len(filtered_references), len(references), language.value)
    ...
    def retrieve_full_file_content(self, relative_file_path: str) -> str:
        server = self._server_for_path(relative_file_path)
        lang = self._language_for_path(relative_file_path)
        log.debug("Retrieving full content of %s using %s language server", relative_file_path, lang.value if lang else "fallback")
    ...
    def insert_text_at_position(self, relative_file_path: str, line: int, column: int, text_to_be_inserted: str) -> Position:
        ...
        lang = self._language_for_path(relative_file_path)
        log.info(
            "Inserting text at %s:%d:%d using %s language server (text length: %d)",
            relative_file_path,
            line,
            column,
            lang.value if lang else "fallback",
            len(text_to_be_inserted),
        )
    ...
    def delete_text_between_positions(self, relative_file_path: str, start: Position, end: Position) -> str:
        ...
        lang = self._language_for_path(relative_file_path)
        log.info(
            "Deleting text in %s from %d:%d to %d:%d using %s language server",
            relative_file_path,
            start["line"],
            start["character"],
            end["line"],
    ...
    def retrieve_content_around_line(
        ...
        lang = self._language_for_path(relative_file_path)
        log.debug(
            "Retrieving content around line %d in %s (before: %d, after: %d) using %s language server",
            line,
            relative_file_path,
    ...
    def search_files_for_pattern(
        ...
        log.info(
            "Searching for pattern '%s' (path: %s, language: %s, include_glob: %s, exclude_glob: %s)",
    ...
    def request_overview(self, within_relative_path: str, language: Language | None = None) -> dict:
        log.debug("Requesting overview for %s (language filter: %s)", within_relative_path, language.value if language else "none")
        ...
        if os.path.isfile(os.path.join(self.repository_root_path, within_relative_path)):
            ...
            if language is None or file_lang == language:
                log.debug(
                    "Getting overview for file %s using %s language server",
                    within_relative_path,
                    file_lang.value if file_lang else "fallback",
                )
                return server.request_overview(within_relative_path)
            else:
                log.debug(
                    "Skipping file %s (language %s doesn't match filter %s)",
                    within_relative_path,
                    file_lang.value if file_lang else "unknown",
                    language.value,
                )
                return {}
